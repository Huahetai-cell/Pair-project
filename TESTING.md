...
## 六、性能分析（内置 -b 基准模式，无需外部探查器）

作业要求"展示一张性能分析图 + 消耗最大的函数"。本工程**不依赖 VS 性能探查器**（配参数 / 附加进程较繁琐），改用"程序内置计时 + 零依赖脚本出图"，两步搞定：

1. 在 exe 目录运行（默认 `-n 200000`，保证采样充分）：
   ```bash
   Myapp.exe -b -r 100 -n 200000
   ```
   生成 `Benchmark.txt`，记录三个阶段（构造树 `build` / 规范化 `canonical` / 格式化 `tostr`）的累计耗时（毫秒），控制台同时打印一张文本柱状图与"消耗最大的函数阶段"。
2. 用 Python（**无需安装任何库**）画图：
   ```bash
   python perf_visualize.py
   ```
   读取 `Benchmark.txt` 输出 `perf_chart.svg`（柱状图 + 占比 + 热点标注）。把这张图贴进博客即可。
3. 若本机没有 Python，直接截图控制台的**文本柱状图**也能满足"展示性能分析图"的要求。

> 说明：`-b` 模式必须带 `-r`（与出题规则一致）；不指定 `-n` 时默认 20 万道，足以得到稳定的占比数据。
